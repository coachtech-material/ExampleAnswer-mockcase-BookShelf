...
今回は、認証不要で誰でもアクセスできる以下の3つのエンドポイントを作成します。

- **書籍一覧取得 API**: `GET /api/v1/books`
- **書籍詳細取得 API**: `GET /api/v1/books/{book}`
- **ジャンル一覧 API**: `GET /api/v1/genres`

レスポンスのJSON構造を整形するために、**APIリソース** を活用します。リソースを再利用することで、コードの重複を防ぎ、保守性を高めます。

### 12.2. 実装手順

#### 1. APIリソースの作成

APIで返すJSONの構造を定義するためのリソースクラスを作成します。

```bash
sail artisan make:resource BookResource
sail artisan make:resource BookCollection
sail artisan make:resource GenreResource
```

#### 2. `GenreResource` の編集

まず、シンプルな `GenreResource` から編集します。`name` のみ返すようにします。
...
次に、`BookResource` を編集します。関連するジャンル情報やレビュー数、平均評価なども含めるようにします。
...
#### 4. `BookCollection` の編集

一覧取得APIでは、ページネーション情報（現在のページ、総ページ数など）も返すのが一般的です。`BookCollection` を編集して、レスポンスにメタデータとリンク情報を追加します。
...
作成した `BookController` に、書籍一覧取得（`index`）と詳細取得（`show`）のロジックを実装します。検索、フィルタリング、ソート機能も追加します。
...
class BookController extends Controller
{
    public function index(Request $request): BookCollection
    {
        ...
        $allowedSortFields = ['created_at', 'title', 'author', 'published_date'];
...
#### 7. APIコントローラーの作成（ジャンル）

次に、ジャンル一覧API用のコントローラーを作成します。

```bash
sail artisan make:controller Api/V1/GenreController
```

#### 8. APIコントローラーの編集（ジャンル）

作成した`GenreController`に、ジャンル一覧取得（`index`）のロジックを実装します。
...
最後に、`routes/api.php` に3つのAPIエンドポイントを定義します。
...
まず、レポート機能のための`ProfileController`を作成します。
...
作成した`ProfileController`に、統計情報を計算する`stats`メソッドを実装します。ここでは、Collectionメソッドを駆使して様々なデータを集計します。
...
**`resources/views/profile/stats.blade.php`**
```blade
<x-app-layout>
    <x-slot name=\"header\">
        <h2 class=\"font-semibold text-xl text-gray-800 leading-tight\">
...
<div class=\"hidden space-x-8 sm:-my-px sm:ml-10 sm:flex\">
    <!-- ... -->
    @auth
        <x-nav-link :href=\"route(\'profile.stats\')\" :active=\"request()->routeIs(\'profile.stats\')\">
            {{ __(\'My Stats\') }}
        </x-nav-link>
    @endauth
</div>
