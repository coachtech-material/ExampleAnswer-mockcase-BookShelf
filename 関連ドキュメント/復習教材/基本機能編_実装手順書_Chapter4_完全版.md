...
> **思考プロセス:**
> なぜ `Route::resource` を使うのでしょうか？ 書籍のCRUDには、一覧表示(index)、登録画面(create)、登録処理(store)、詳細表示(show)、編集画面(edit)、更新処理(update)、削除処理(destroy) の計7つのアクションが必要です。これらを一つずつ定義するのは冗長です。
> 
...
> - `store()`: `collect($validated)->except(["genres"])` で、バリデーション済みデータから`genres`キーを除外した書籍本体のデータを作成しています。その後、`create`で書籍を保存し、`attach`で中間テーブルにジャンルとの関連を保存します。
> - `show()`: `load()`メソッドで、書籍詳細ページで必要になるリレーション（レビュー投稿者、レビューへのいいね、ジャンル）をまとめてEager Loadingしています。
> - `update()`: `store`と同様に書籍本体のデータとジャンルのデータを分離し、`update`で書籍情報を更新後、`sync`でジャンルの関連を同期（古い関連を削除し、新しいものを登録）します。
