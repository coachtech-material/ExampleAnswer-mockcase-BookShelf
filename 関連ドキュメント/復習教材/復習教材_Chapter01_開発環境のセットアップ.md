...
> **⚠️ 日本語化は外部パッケージを使わない**
> 日本語化は上記の `lang/ja/` への手動配置のみで行います。`laravel-lang/lang` などの `laravel-lang/*` 系パッケージ（`composer require laravel-lang/...`）は導入しないでください。同系パッケージは 2026年5月のサプライチェーン攻撃で一部バージョンがマルウェア配布に悪用された経緯があり、本課題は `lang/ja/` で完結するため外部依存は不要です。
