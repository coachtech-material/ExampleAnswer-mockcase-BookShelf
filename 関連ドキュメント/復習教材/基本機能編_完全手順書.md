...
| GET | `/genres/{genre}` | `GenreController@show` | ジャンル別書籍一覧 | 必要 |
...
> **ポイント:** 基本版では `HasApiTokens` トレイトは不要。リレーションメソッドに戻り値の型宣言（`: HasMany` 等）は付けない。PHPDocコメントも不要。`belongsToMany` に `withTimestamps()` は付けない（中間テーブルは複合主キー構造で timestamps カラムを持たないため）。
...
> **ポイント:** 固定の `$reviews` 配列では (book_id, user_id) の組が一意になっているため、`create` で各レビューをそのまま挿入している。評価は3〜5に偏ったデータになっている。
...
> **ポイント:** 基本版では `comment` は必須入力（NOT NULL）のため、StoreReviewRequest / UpdateReviewRequest で `required` バリデーションを適用して未入力を弾く。エラーメッセージ（`comment.required`）も忘れずに定義すること。
