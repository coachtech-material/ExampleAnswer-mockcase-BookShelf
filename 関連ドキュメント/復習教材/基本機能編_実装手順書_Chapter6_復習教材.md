...
### 6.2.2. BookControllerの実装 (権限チェックなし)

次に、`BookController`に、権限チェックを**コメントアウトした状態**でCRUD処理を実装します。
...
作成された`app/Policies/BookPolicy.php`に、更新（`update`）と削除（`delete`）の権限ルールを記述します。
