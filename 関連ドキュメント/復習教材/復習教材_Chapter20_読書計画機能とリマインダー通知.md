...
> **📌 読書計画機能を実装していない方へ（最初にお読みください）**
>
> 読書計画機能（本 Chapter）は **2026年5月上旬に追加された応用要件**です。それより前にテストを開始した方の要件定義書には含まれていないため、**実装していなくても問題ありません**（要件外のため採点対象外です）。
>
> ただし、本 Chapter で扱う技術 ― **PHP Enum + Eloquent cast** / **Eloquent scope** / **DB::transaction** / **Schedule + Console Command による日次バッチ** / **Notification（DatabaseChannel）による通知** ― は、**次の「模擬案件2」では基礎機能として登場します**。実装の有無にかかわらず、本 Chapter は飛ばさず必ず一度読み込んでおきましょう。
