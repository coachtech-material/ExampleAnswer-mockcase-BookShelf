...
50% 要件シートの「7. 機能要件一覧」を見て、必要なデータを予測し、Blade のナビゲーションで裏付けを取ります。

#### 1. 手がかり1: 要件シート (7. 機能要件一覧)
...
#### 1. 手がかり1: 要件シート (7. 機能要件一覧 / 書籍登録・書籍詳細)

* **記載内容**: 「書籍登録：タイトル、著者、ISBN、出版日、説明、画像URL...」
* **エンジニアの思考**:
    * 「項目は分かった。だが、ISBN の桁数や、必須かどうかの記載がない。」

#### 2. 手がかり2: Blade (_form.blade.php / reviews/edit.blade.php)

* **ISBN**: プレースホルダー `978...`（ハイフンなし）
* **説明**: プレースホルダーに `（任意）` と記載あり。
* **評価**: ラジオボタンで `1` ~ `5` のみ。
* **エンジニアの思考**:
    * 「ISBN は 13 桁だから文字列として扱おう。」
    * 「説明は『任意』だから、DB側でも `NULL` を許容（Nullable）しないとエラーになる。」
    * 「評価は整数のみだから、`INTEGER` ではなく最小の `TINYINT` で十分だ。」

#### PMへのヒアリング例 (Q3, Q4, Q5)

* 「ISBN は画面に合わせて**13 桁・ハイフンなし**とし、DB 定義は `VARCHAR(13)` とします。また、システムとして**一意（Unique）**制約をかけますか？」
...
#### 1. 手がかり1: 要件シート (7. 機能要件一覧 / ジャンル一覧)

* **記載内容**: 「ジャンル削除：紐付きがある場合は削除を制限すること」
* **エンジニアの思考**:
    * 「要件書にサラッと重要な制約（制限事項）が書いてある。『紐付きがある場合は』ということは、無条件で削除してはいけない。」
...
* **解説**:
  ...
  * **ジャンル削除**: 使用中は消してはいけない（Restrict）
  このように、データの種類に応じて適切な削除ルールを定義するのがエンジニアの責任です。
...
| プレースホルダー `978...`（ハイフンなし） | ISBN は `VARCHAR(13)`, UNIQUE |
| プレースホルダーに「(任意)」 | `description`, `image_url` は nullable |
| ラジオボタン 1~5 | `rating` は `TINYINT` |
| 「紐付きがある場合は削除を制限すること」 | ジャンル削除は Restrict（コントローラーでチェック） |
