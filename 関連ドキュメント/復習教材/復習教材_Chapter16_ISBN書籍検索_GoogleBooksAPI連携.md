...
### 思考1：バックエンドとフロントエンドの役割をどう分けるか？

> 「今回の機能は、①フロントエンド（ブラウザ）がISBNをバックエンドに送り、②バックエンドがGoogle APIと通信して結果を返し、③フロントエンドがその結果をフォームに反映させる、という流れだ。この役割分担を明確にすることが大事。バックエンドはAPIとの通信とデータ整形に専念し、フロントエンドはUIの操作と非同期通信に専念させよう。つまり、Laravel側はJSONを返すAPIエンドポイント（例：`/books/isbn/{isbn}`）を用意すればいい。」
