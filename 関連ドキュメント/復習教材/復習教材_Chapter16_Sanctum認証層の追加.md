...
> **画面動作確認時の注意:** ブラウザで認証が必要な画面（書籍一覧 / 詳細 / ランキング等）にアクセスすると `Route [reports.index] not defined` で 500 エラーになります。`reports.index` ルートは **Chapter 19 マイレポート機能** で定義されるため、画面動作確認は Chapter 19 完了後に行ってください。本 Chapter の動作確認は API エンドポイントの curl による確認に絞ります。
...
認証と認可を適切に設定することで、APIのセキュリティが大幅に向上しました。次の Chapter 17 では、書籍一覧画面に検索・フィルタ・並び替えを加える**高度な検索機能**を実装します。
