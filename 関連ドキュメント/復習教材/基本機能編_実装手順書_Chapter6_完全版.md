...
> **思考プロセス:**
> なぜ`ReviewController`には`--resource`オプションを付けないのでしょうか？ 今回実装するレビュー機能は、投稿（`store`）、更新（`update`）、削除（`destroy`）が主であり、レビューの一覧（`index`）は書籍詳細ページの一部として表示され、独立したページは持ちません。このように、RESTfulなリソースの7つのアクション全てを必要としない場合は、`--resource`を使わずにコントローラを作成し、必要なメソッドだけを個別に定義する方が、不要なコードが生成されず、プロジェクトをクリーンに保てます。
...
public function rules(): array
{
    return [
        ...
        "comment" => "required|string|max:1000", // 模範解答ではnullableではない
    ];
}
```
...
> **コード解説:**
> - `store()`: `Book`モデルと`Review`モデルのリレーション（`hasMany`）を利用して、`$book->reviews()->create(...)` とすることで、`book_id`と`user_id`を自動的に設定しながらレビューを保存しています。模範解答ではこの形式で実装されています。
> - `update()`: 更新後、`$review->book` を使って関連する書籍モデルを取得し、その書籍の詳細ページにリダイレクトしています。
