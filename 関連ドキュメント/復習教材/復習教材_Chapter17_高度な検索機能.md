# Chapter 17: 高度な検索機能の実装

## 🎯 このセクションで学ぶこと

基本機能編で実装した「タイトル / 著者の単純検索」を拡張し、実務でよく求められる**絞り込み + ソート + ページネーション**を 1 つのエンドポイント `BookController@index` で扱う「高度な検索機能」を実装します。

- **クエリパラメータ駆動の検索**: URL の `?keyword=...&genre_id=...&sort=...` から条件を受け取り、`when()` / `whereHas` で動的にクエリを組み立てる
- **絞り込み**: タイトル・著者の部分一致 (`LIKE`) と、ジャンル（多対多リレーション越し）の絞り込みを組み合わせる
- **ソート**: 新しい順 / 古い順 / タイトル昇順 / 平均評価の高い順を `switch` 式で分岐
- **ページネーション + 条件保持**: `paginate(10)` の `appends(request()->query())` で「次へ」ボタンが検索条件を引き継ぐ
- **フォーム入力値の復元**: 検索実行後も検索フォームに入力した値・選択中のジャンルが残るパターン
...
`when($condition, fn ($query) => ...)` は「条件 A が truthy なら、続けてクエリを組み立てる」という条件分岐つきのクエリビルダ書き方。`if ($condition) { $query->where(...) }` を書くより読みやすく、メソッドチェーンで完結できる。

### `whereHas` で関連先の条件を指定

`whereHas('genres', fn ($q) => $q->where('genres.id', $genreId))` は「ジャンル ID = $genreId に該当する関連先を持つ Book だけに絞り込む」という意味。JOIN を書かずに「関連先の条件」を表現できる。

### `appends(request()->query())` の役割

ページネーションリンク（「次へ」「2」「3」等）が生成されるとき、デフォルトでは現在の URL パラメータが引き継がれない。`appends(request()->query())` で「現在のクエリパラメータをページネーションリンクに足す」ことで、検索結果のページ遷移時に絞り込み条件が維持される。
...
> **画面動作確認時の注意:** 本 Chapter の動作確認はすべて書籍一覧画面（`/books`）の操作です。`/books` をはじめとする認証必須でない画面でも `navigation.blade.php` が `reports.index` を参照しているため、Chapter 20 読書計画 + 通知の実装前は `Route [reports.index] not defined` で 500 エラーになります。**画面動作確認は Chapter 21 完了後に行ってください。** 本 Chapter 完了時点では、`BookController@index` のコード変更と Blade（高度な検索フォーム）の配置までを終え、Chapter 21 完了後にまとめて画面動作確認するのが推奨フローです。

| 確認項目 | 確認方法 |
|:---|:---|
| キーワード検索 | `/books?keyword=Laravel` のように URL を直接叩く or 検索フォームでキーワード入力 → タイトル or 著者に含まれる書籍だけ表示される |
| ジャンル絞り込み | ジャンルプルダウンを選択 → 該当ジャンルに属する書籍だけ表示される |
| 並び替え | 並び順を「新しい順」「古い順」「タイトル昇順」「評価が高い順」に切り替えて、リスト順が変わること |
| 複合条件 | キーワード + ジャンル + 並び替え を同時指定して、すべての条件が AND で効くこと |
| 検索条件のフォーム保持 | 検索実行後、フォームに入力した値（keyword / 選択中のジャンル / sort）が表示維持される |
| ページネーションの条件引継ぎ | 検索結果が 10 件超のとき「次へ」を押すと URL に検索条件が引き継がれていること（`?keyword=...&page=2` のような形） |
