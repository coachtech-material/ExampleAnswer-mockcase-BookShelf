...
| books | id, user_id(FK), title, author, isbn(unique), published_date, description(nullable), image_url(nullable) | user_id CASCADE。**isbn は基本・応用とも NOT NULL、published_date は基本で NOT NULL・応用で nullable**（ISBN 検索で出版日を取得できないケースを許容するため。isbn はユニークキーのため NOT NULL を維持） | 基本+応用 |
...
> - `books.published_date` を `nullable` に変更する（Part 3 Step 18 ISBN 検索で出版日を取得できないケースの登録を許容するため。`books.isbn` はユニークキーのため NOT NULL を維持する）
...
ポイント: `isbn` は基本・応用とも `required`。`published_date` は基本で `required`（応用機能編 Step 18 ISBN 検索の対応で nullable に変更する）。Web 版では `genres` に `min:1` は不要（HTML フォームでチェックボックス未選択時は `genres` キー自体が送られないため `required` のみで弾ける）。API 版（Step 13）では `genres` に `min:1` を付与する。
...
ジャンル別書籍一覧は `GenreController@show` で実装する。Step 11 のジャンル管理 CRUD 実装に show メソッドを含めて先行実装済みのため、本 Step では動作確認のみ行う:

- `/genres/{genre}` にアクセスしてそのジャンルに属する書籍が一覧表示される
- 認証不要（`Route::middleware('auth')->group(...)` の外で定義）
- 紐付く書籍が無いジャンルでも画面が正常に表示される
- 存在しない ID では 404 が返る
...
> **重要:** 提供された Blade テンプレート（navigation.blade.php 等）は、応用機能編で追加する `reports.index` / `reading-plans.index` / `notifications.index` ルートを参照しています。テストファイルはこの Step で作成しますが、**`sail artisan test` の実行は Step 21 完了後に行ってください。** `reports.index` / `reading-plans.index` / `notifications.index` ルートが未定義の状態でテストを実行すると、ビューのレンダリングで `Route [reports.index] not defined` エラーが発生します（実際の実行は Step 21 で応用機能のテストを追加した後に行うのが自然）。
...
class BookModelTest extends TestCase
{
    ...
    public function test_book_relationships_are_defined()
    {
        $user = User::factory()->create();
...
class ReviewModelTest extends TestCase
{
    ...
    public function test_review_relationships_are_defined()
    {
...
class BookRequestTest extends TestCase
{
    ...
    public function test_isbnは13桁でなければならない()
    {
        $user = User::factory()->create();
        $genre = Genre::factory()->create();
...
class BookControllerTest extends TestCase
{
    ...
    public function test_book_index_page_can_be_rendered()
    {
    ...
    public function test_book_show_page_can_be_rendered()
    {
    ...
    public function test_only_owner_can_view_edit_form()
    {
        $owner = User::factory()->create();
        $book = Book::factory()->for($owner)->create();
        $otherUser = User::factory()->create();
...
class ReviewTest extends TestCase
{
    ...
    public function test_review_store_validation_errors()
    {
        $user = User::factory()->create();
        $book = Book::factory()->create();
    ...
    public function test_only_owner_can_edit_review()
    {
        $owner = User::factory()->create();
    ...
    public function test_only_owner_can_update_review()
    {
        $owner = User::factory()->create();
    ...
    public function test_only_owner_can_delete_review()
    {
        $owner = User::factory()->create();
        $review = Review::factory()->for($owner)->create();
        $otherUser = User::factory()->create();
...
class FavoriteTest extends TestCase
{
    ...
    public function test_favorite_toggle_works_correctly()
    {
        $user = User::factory()->create();
        $book = Book::factory()->create();
        $from = route('books.show', $book);
    ...
    public function test_favorite_index_page_can_be_rendered()
    {
        $user = User::factory()->create();
...
class ReviewLikeTest extends TestCase
{
    ...
    public function test_user_can_like_review()
    {
        $user = User::factory()->create();
    ...
    public function test_user_can_unlike_review()
    {
        $user = User::factory()->create();
        $review = Review::factory()->create();
    ...
    public function test_guest_cannot_like_review()
    {
        $review = Review::factory()->create();
...
class GenreTest extends TestCase
{
    ...
    public function test_genre_index_page_can_be_rendered()
    {
        $user = User::factory()->create();
    ...
    public function test_genre_store_validation_errors()
    {
        $user = User::factory()->create();
    ...
    public function test_genre_show_page_displays_books()
    {
        $user = User::factory()->create();
    ...
    public function test_genre_without_books_can_be_deleted()
    {
        $user = User::factory()->create();
...
class RankingTest extends TestCase
{
    ...
    public function test_ranking_page_can_be_rendered()
    {
...
class RedirectIfAuthenticatedTest extends TestCase
{
    ...
    public function test_guest_can_access_route()
    {
        $middleware = new RedirectIfAuthenticated;
        $request = Request::create('/login', 'GET');
...
class ReviewPolicyTest extends TestCase
{
    ...
    public function test_レビュー投稿者のみが編集できる()
    {
        $owner = User::factory()->create();
    ...
    public function test_レビュー投稿者のみが削除できる()
    {
        $owner = User::factory()->create();
        $other = User::factory()->create();
        $genre = Genre::factory()->create();
...
class BookApiTest extends TestCase
{
    ...
    public function test_index_returns_data_and_meta_structure()
    {
        $user = User::factory()->create();
        $genre = Genre::factory()->create();
    ...
    public function test_index_pagination_per_page()
    {
        $user = User::factory()->create();
        $genre = Genre::factory()->create();
    ...
    public function test_index_returns_422_when_per_page_exceeds_max()
    {
    ...
    public function test_show_returns_book_with_reviews()
    {
        $user = User::factory()->create();
        $genre = Genre::factory()->create();
        $book = Book::factory()->for($user)->create();
    ...
    public function test_show_returns_custom_404_json_when_not_found()
    {
    ...
    public function test_store_creates_book_and_attaches_genres()
    {
        $user = User::factory()->create();
        $genres = Genre::factory()->count(2)->create();
    ...
    public function test_update_modifies_book_fields_and_genres()
    {
        $user = User::factory()->create();
    ...
    public function test_update_returns_404_for_unknown_book()
    {
        $user = User::factory()->create();
        $genre = Genre::factory()->create();
    ...
    public function test_update_returns_422_with_validation_errors()
    {
        $user = User::factory()->create();
        $book = Book::factory()->for($user)->create();
    ...
    public function test_destroy_returns_404_for_unknown_book()
    {
...
> **注意:** Step 14 完了時点では `sail artisan test` を実行しない。`reports.index` ルートが未定義のため、ビュー描画を含むテストが軒並み失敗する。実行は **Step 21 完了後**（マイレポート機能実装後）に行う。実際の実行タイミングは Step 21 で応用機能テストを追加した後がおすすめ。
...
### 提供 Blade ファイル（Advanced 用）の再取得

Part 2 Step 4.7 で配置した **Basic ブランチ** の Blade を、**Advanced ブランチ** の Blade に上書きする。これにより応用機能用の Blade（マイレポート画面 / 読書計画画面 / 通知画面 / 高度な検索フォーム / ISBN 検索ボタン等）が利用可能になる。
...
> **注:** Basic ブランチで先に動かしていた認証・書籍・ジャンル等の Blade も Advanced 版に置き換わるが、Advanced 用 Blade は Basic 機能と互換のため動作が壊れることはない。
...
`$fillable` / `$casts` 上に PHPDoc を追加。`$casts` に `published_date` を追加。リレーションメソッドの戻り値型宣言と PHPDoc を追加。
...
Part 2 段階では型宣言を付与していないため、`StoreReviewRequest` / `UpdateReviewRequest` 等の各 FormRequest クラスのメソッドに、戻り値型（`: bool`、`: array`）と PHPDoc を追加する。
...
Part 2 で作成したコントローラ群（`BookController`, `ReviewController`, `FavoriteController`, `ReviewLikeController`, `GenreController`, `RankingController`, `Api\V1\BookController` 等）の各アクションメソッドに、戻り値型（`: View`、`: RedirectResponse`、`: JsonResponse` 等）と、目的を 1 行で説明する PHPDoc を追加する。`Api\V1\BookResource` 等の Resource クラスの `toArray()` メソッドにも `: array` 戻り値型を追加する。Policy / Exception Handler も同様。
...
> **注:** 本セクションは Part 2 で実装したコードへの「品質追加」の手順です。各既存ファイルを上記の変更内容に従って編集してください。**`sail artisan test` の実行は Step 21 完了後**（`reports.index` ルートが定義された後）に行うため、ここでは型変更後に構文エラーがないことを確認するのみで Step 16 に進んでください。
...
> **画面動作確認時によくあるエラー:**
>
> - 「読書計画」リンクで `Call to undefined method App\Models\User::readingPlans()` → Step 20.4 末尾の `app/Models/User.php` への `readingPlans()` リレーション追記が未実施。
> - 「通知」リンクで未読件数バッジが出ない → 日次バッチ `sail artisan reading-plans:run-daily` を実行したか確認。
> - `reading_plans` テーブルや `notifications` テーブルが無い → `sail artisan migrate:fresh --seed` を実行したか、または Step 20.1 / 20.2 のマイグレーションファイルが正しく配置されているか確認。
...
> **よくあるエラーと対処:**
>
> - `Call to undefined method App\Models\User::withAccessToken()` → Step 16.2「User モデルへの HasApiTokens トレイト追加」が未実施。`app/Models/User.php` の `use Laravel\Sanctum\HasApiTokens;` と `use HasApiTokens, HasFactory, Notifiable;` を確認。
> - `Call to undefined method App\Models\User::readingPlans()` → Step 20.4 末尾の User モデルへの `readingPlans()` リレーション追記が未実施。
> - `Route [reports.index] not defined` → Step 19 マイレポート機能が未実装、または Step 19 完了前に test 実行している。Step 19 を完了させてから再実行する。
> - `Constant PDO::MYSQL_ATTR_SSL_CA is deprecated since 8.5` → PHP 8.5 ベースの sail コンテナで発生する deprecated 警告。テストは pass するので無視可。気になる場合は `config/database.php` の `PDO::MYSQL_ATTR_SSL_CA` を `Pdo\Mysql::ATTR_SSL_CA` に書き換える（PHP 8.4+ のみ。書き換え後 `Class "Mysql" not found` エラーが出る環境では元に戻す）。

### 22.3. README.md の作成

要件シート L68-75「README.md 記載必須項目」に従い、以下を含む README.md を作成する。**README が不十分だと採点者が環境構築・機能確認できないため、再提出または大きな減点の対象になる**。
