...
Route::middleware('auth')->group(function () {
    ...
    // 書籍管理 (Resourceコントローラーを使用、showは公開側で定義済みなので除外)
    // create, store, edit, update, destroy が自動定義されます
    Route::resource('books', BookController::class)->except(['index', 'show']);

    // ジャンル管理 (Resourceコントローラーを使用、showは公開側で定義済みなので除外)
...
### 7.5. コントローラーへの権限チェック追加から




### 6.3. ルートの定義 (routes/web.php)

書籍管理に関するルートを定義します。認証が必要なルートと不要なルートを適切に分けます。
``routes/web.php``を修正していきます。
...
// 認証用ルート (auth.phpの読み込み)
require __DIR__.'/auth.php';
```

### 6.4. BookController.php の実装 (権限チェックなし)

まずはCRUD機能が動作するようにコントローラーを実装します。 
この段階では authorize メソッドを使用しないため、ログインユーザーであれば誰でも編集・削除が可能な状態となります。  
``app/Http/Controllers/BookController.php``を修正していきます。
...
class BookController extends Controller
{
    public function index(): View
    {
        // 書籍一覧を最新順に取得し、ジャンル情報をEager Loadingする
        $books = Book::with("genres")->latest()->paginate(10);
        return view("books.index", compact("books"));
    }
    ...
    public function update(UpdateBookRequest $request, Book $book): RedirectResponse
    {
        ...
        // ジャンルの紐付けを同期（選択されていないものは削除、新しいものは追加）
        $book->genres()->sync($request->genres);
...
```
ここで動作確認を行ってください ブラウザで /books/create にアクセスして書籍を登録できるか、詳細画面が表示されるか、編集・削除ができるかを確認してください。問題なく動作するはずです。

### 6.5. ポリシーの作成と登録
CRUD機能の動作確認ができたら、セキュリティを強化します。「投稿者本人だけが編集・削除できる」というルールを実装します。
...
作成されたファイルを開き、``update`` と ``delete`` メソッドにロジックを記述します。
...
class BookPolicy
{
    // 更新権限：ユーザーIDと書籍の投稿者IDが一致する場合のみ許可
    public function update(User $user, Book $book): bool
    {
        return $user->id === $book->user_id;
    }

    // 削除権限：ユーザーIDと書籍の投稿者IDが一致する場合のみ許可
    public function delete(User $user, Book $book): bool
    {
        return $user->id === $book->user_id;
    }
...
3. ポリシーの登録 (``app/Providers/AuthServiceProvider.php``)
作成したポリシーをアプリケーションに登録します。
```php
// ...
use App\Models\Book; // 追加
use App\Policies\BookPolicy; // 追加
...
```
### 6.6. コントローラーへの権限チェック追加
最後に、``BookController.php`` に権限チェックのコードを追加して完成させます。  
``app/Http/Controllers/BookController.php`` の以下のメソッドを修正:
...
これで、他のユーザーが投稿した書籍を編集・削除しようとすると「403 Forbidden」エラーが表示されるようになり、安全な実装が完了しました。

## Step 7: レビュー機能

### 7.1. コントローラーとリクエストの作成
...
### 10.4. ランキングビュー (`resources/views/ranking/index.blade.php`)
...
### 12.1. GenreControllerにshowメソッドを追加
...
## Step 13: ジャンル管理機能 (CRUD)

### 13.1. GenreControllerの完全実装
