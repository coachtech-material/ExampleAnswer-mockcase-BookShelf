...
| book_genre | book_id(FK), genre_id(FK) | 中間テーブル、複合主キー(book_id, genre_id) |
| reviews | id, user_id(FK), book_id(FK), rating(tinyInt), comment(NOT NULL) | CASCADE |
| favorites | user_id(FK), book_id(FK) | 中間テーブル、複合主キー(user_id, book_id) |
| review_likes | user_id(FK), review_id(FK) | 中間テーブル、複合主キー(user_id, review_id) |
