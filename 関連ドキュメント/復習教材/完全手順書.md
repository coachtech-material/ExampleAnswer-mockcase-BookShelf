...
- **Advance 学習者**: Part 1 と Part 2 を進めた後、Part 3（Step 15〜23）に進むことで、応用版（advanced ブランチ）の模範解答コードと同一のものが完成します。Part 3 では既存コードへの品質強化（型宣言・PHPDoc・Collection 多用）と応用機能（ISBN検索・高度な検索・CSV エクスポート・マイ読書レポート・読書計画+リマインダー通知・Sanctum 認証API）を実装します。
...
| favorites | user_id(FK), book_id(FK) | 中間テーブル、複合主キー(user_id, book_id)、timestamps なし、user_id CASCADE、book_id CASCADE | 基本 |
| review_likes | user_id(FK), review_id(FK) | 中間テーブル、複合主キー(user_id, review_id)、timestamps なし、user_id CASCADE、review_id CASCADE | 基本 |
...
> - `books.isbn` と `books.published_date` を `nullable` に変更する（Part 3 Step 19 ISBN 検索で部分情報のみの登録を許容するため）
...
ポイント: `id` カラムなし、`timestamps` なし。複合主キー `primary(['user_id', 'book_id'])` を使用する。
...
ポイント: `id` カラムなし、`timestamps` なし。複合主キー `primary(['user_id', 'review_id'])` を使用する。
...
> **注意:** `Favorite` モデルと `ReviewLike` モデルは作成しない。中間テーブル `favorites` / `review_likes` へのアクセスは、User / Book / Review に定義した `belongsToMany` リレーション経由で行う（`favoriteBooks` / `favoritedByUsers` / `likedReviews` / `likedByUsers`）。
...
ポイント: `isbn` と `published_date` は `required`（応用機能編 Step 19 ISBN 検索の対応で nullable に変更する）。Web 版では `genres` に `min:1` は不要（HTML フォームでチェックボックス未選択時は `genres` キー自体が送られないため `required` のみで弾ける）。API 版（Step 13）では `genres` に `min:1` を付与する。
...
## Step 18: CSVエクスポート機能（応用機能）

> ⚠️ **TODO:** CSV エクスポート機能（書籍一覧画面に「CSV ダウンロード」ボタンを追加し、現在表示中の書籍一覧を CSV ファイルとしてダウンロードできるようにする）は応用機能編で予定されている機能ですが、本セッション時点では advanced ブランチの模範解答コードに実装が存在しません。詳細な実装手順は `関連ドキュメント/復習教材/復習教材_Chapter18_CSVエクスポート機能.md` を参照してください。模範解答コードへの実装後、本 Step に手順を追記する想定です。
>
> 実装完了まで本 Step はスキップしても構いません（CSV エクスポート機能なしで Step 19 ISBN 検索に進める）。

---

## Step 19: ISBN書籍検索（Google Books API連携）（応用機能）

### 19.1. Google Books API 設定
...
### 19.2. BookController に ISBN 検索メソッドを追加
...
## Step 20: マイ読書レポート機能（応用機能）

### 20.1. `app/Http/Controllers/ReportController.php`
...
### 21.1. `database/migrations/xxxx_create_reading_plans_table.php`
...
### 21.2. `database/migrations/xxxx_create_notifications_table.php`
...
### 21.3. `app/Enums/ReadingPlanStatus.php`
...
### 21.4. `app/Models/ReadingPlan.php`
...
### 21.5. `database/factories/ReadingPlanFactory.php`
...
### 21.6. `app/Policies/ReadingPlanPolicy.php` と AuthServiceProvider 登録
...
### 21.7. `app/Http/Requests/{Store,Update}ReadingPlanRequest.php`
...
### 21.8. `app/Notifications/PlanReminderNotification.php`
...
### 21.9. `app/Console/Commands/RunReadingPlanDailyBatch.php` と Console Kernel
...
### 21.10. `app/Http/Controllers/ReadingPlanController.php`
...
### 21.11. `app/Http/Controllers/NotificationController.php`
...
### 21.13. `resources/views/layouts/navigation.blade.php`（提供済の確認）
...
### 21.14. `database/seeders/ReadingPlanSeeder.php` と DatabaseSeeder 登録
...
### 22.3. BookApiTest（Sanctum認証テスト含む）
...
### 22.5. ReadingPlanDeadlineChangeTest
...
### 22.6. ReadingPlanReminderBatchTest
...
### 22.7. ReadingPlanAutoExpireBatchTest
