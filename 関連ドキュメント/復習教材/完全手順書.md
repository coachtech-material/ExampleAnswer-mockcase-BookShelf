...
配布されたBladeファイル（ビュー）には、すでにリンク先として多数のルート名（例: ``route('books.index')``）が記述されています。 これらが ``web.php`` に定義されていないと、画面を表示した瞬間にエラーとなってしまいます。 そこで、機能ごとの詳細な実装に入る前に、必要なすべてのコントローラーを作成し、ルート定義を完了させます。
