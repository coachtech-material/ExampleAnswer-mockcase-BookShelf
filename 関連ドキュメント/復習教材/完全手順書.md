...
`app/Http/Controllers/RankingController.php` の集計ロジックを Collection メソッドで書き直す。生 SQL 集計を Eloquent + Collection でリファクタすることで、ジャンル別フィルタや拡張に強くなる。

具体的なリファクタ内容は `RankingController` の Basic 実装と Advance 実装を見比べながら、Eloquent クエリビルダ + Collection の `map` / `sortByDesc` を活用する形に書き換える（詳細実装は応用機能編 模範解答コードを参照）。
