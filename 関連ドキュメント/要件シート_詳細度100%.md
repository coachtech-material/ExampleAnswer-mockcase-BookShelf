...
| テーブル設計: カラム名・型・制約・ER図 | 「中間テーブル（book_genre）は複合主キー構成、favorites / review_likes は『いいね』『お気に入り』というエンティティとして扱うため、サロゲートキー（id）+ 複合 unique 制約で設計しましたが、この方針で問題ないですか？」<br>「外部キーにはON DELETE CASCADEを設定する想定ですが、カスケード削除で合っていますか？」<br>「レビューにuser_idとbook_idの一意制約を付けて1ユーザー1書籍1件に制限する設計にしましたが、この制約は必要ですか？」 | テーブル仕様書 — 全テーブルのカラム定義・制約・ER図を記載 | 基本 |
...
| ★ 読書計画機能の設計: 計画CRUD・状態遷移・期限変更 | 「読書計画は1書籍1計画で、書籍プルダウンから選択する設計にしましたが、この方針で問題ないですか？」<br>「進捗管理は『読了する』ボタンのみで、% 入力は不要にする設計にしましたが、この方針で合っていますか？」<br>「同一書籍に in_progress 状態の他計画があれば作成不可、completed 後は再計画可とする設計にしましたが、この制約で問題ないですか？」<br>「期日変更時に Expired 計画は in_progress に復帰、completed 計画は編集不可とする設計にしましたが、この挙動で合っていますか？」<br>「計画削除時に関連リマインダー通知も同時削除する設計にしましたが、この方針で問題ないですか？」 | 機能要件 — 読書計画一覧/作成/編集画面の各操作行 + バリデーションルール — StoreReadingPlanRequest / UpdateReadingPlanRequest | ★ 応用 |
| ★ リマインダー通知の設計: 発火タイミング・条件・チャネル | 「リマインダーは期日 3 日前 / 当日 / 3 日後 の 3 タイミングで毎日 20:00 に発火する設計にしましたが、この方針で問題ないですか？」<br>「3 日後の通知は status=Expired の計画に対する『再エンゲージメント』通知として発火する設計にしましたが、この役割設計で合っていますか？」<br>「通知は DatabaseChannel のみ採用し、メール送信はしない設計にしましたが、このチャネル選択で合っていますか？」<br>「読了完了時の通知は発火せず、フラッシュメッセージのみで完結する設計にしましたが、この方針で問題ないですか？」 | 機能要件 — 日次バッチ処理の行 + シート3 の Schedule 登録手順 | ★ 応用 |
| ★ 通知UIの設計: ベルアイコン・既読管理 | 「ヘッダー右側にベルアイコンと未読件数バッジを配置し、クリックで /notifications に遷移する設計にしましたが、この導線で合っていますか？」<br>「通知データに plan_id / book_title / timing / title / body の 5 フィールドを格納する設計にしましたが、この構造で問題ないですか？」<br>「既読化は『既読にする』ボタンで個別実行する設計にしましたが、この UX で問題ないですか？」 | 機能要件 — 通知一覧画面の各操作行 + テーブル仕様書 — notifications テーブルの data カラム説明 | ★ 応用 |
| ★ Enum と Eloquent scope の設計 | 「ReadingPlanStatus PHP Enum でケース名を InProgress / Completed / Expired と定義し、reading_plans.status に Eloquent cast を適用する設計にしましたが、このケース名で問題ないですか？」<br>「Eloquent scope を active / completed / expired の 3 種で定義し、一覧での絞り込みに使用する設計にしましたが、この scope 名で合っていますか？」 | テーブル仕様書 — reading_plans.status カラムの補足欄 + 機能要件 — 一覧画面の使用技術列 | ★ 応用 |
...
| 環境 | 技術スタック | OS（Dockerが動作する任意のOS）: -<br>PHP: 8.2<br>Laravel: 10.x<br>DB: MySQL 8.4<br>フロントエンド: Vite, Tailwind CSS ^3.4.0, @tailwindcss/forms<br>開発ツール: Docker, Laravel Sail, phpMyAdmin | | 基本 |
...
| | ★ Sanctum APIトークン認証 | ・公開API の書き込み系エンドポイント（POST/PUT/DELETE）には Laravel Sanctum によるトークン認証を導入する<br>・`auth:sanctum` ミドルウェアで認証必須化し、BookPolicy で書籍所有者のみ更新・削除できるよう認可する | 基本機能では公開API は読み取り（GET 一覧/詳細）と認証なしの CRUD として実装し、応用機能で Sanctum を後付けして本物のセキュリティ層を追加する | ★ 応用 |
| | ★ Schedule + Console Command | ・読書計画のリマインダー通知 + Auto-expire を毎日 20:00 に実行する日次バッチを Console Kernel の `schedule()` メソッドに登録する<br>・Console Command（`reading-plans:run-daily`）として実装する | 開発環境では `sail artisan schedule:work` で動作確認 | ★ 応用 |
| | ★ Database Transaction | ・複数の SQL 文を 1 単位で扱う処理は `DB::transaction()` でラップする<br>・読書計画削除時に関連リマインダー通知も同時に削除する処理（複数 DELETE）で使用する | DB 整合性を保つ。単一の `update()` は内部で 1 つの SQL UPDATE 文として実行されるため Transaction は不要 | ★ 応用 |
| | ★ Notification facade（DatabaseChannel） | ・読書計画リマインダーは Laravel 標準の Notification facade を使用し、DatabaseChannel に通知を発火する<br>・User モデルが Notifiable trait を持ち（Laravel デフォルト）、`$plan->user->notify(new PlanReminderNotification($plan, $timing))` で発火する | メールチャネルは採用しない | ★ 応用 |
| | ★ PHP Enum + Eloquent cast | ・status のような限定された値の集合は PHP 8.1+ ネイティブ Enum で定義し、Eloquent の `casts()` でカラムに紐付ける<br>・`match` 式で網羅的な分岐を書く（label() メソッド等） | 型安全性とコードの可読性を担保 | ★ 応用 |
...
※M1/M2/M3 Mac（Apple Silicon）をお使いの方：`sail up -d` 実行時に `no matching manifest for linux/arm64/v8` エラーが発生した場合、`compose.yaml` の `mysql` サービスに `platform: 'linux/amd64'` を追加してください。
...
本プロジェクトでは、フロントエンドのスタイリングに Tailwind CSS を使用します。以下の手順でセットアップを行ってください。
...
**3. Tailwind CSS と `@tailwindcss/forms` プラグインのインストール**

```bash
sail npm install -D tailwindcss@^3.4.0 @tailwindcss/forms postcss autoprefixer
```

※ `@tailwindcss/forms` はフォーム要素のスタイルをリセットする Laravel 標準プラグインです。
...
**5. Tailwind CSS のテンプレートパス設定と forms プラグインの有効化**
...
**6. resources ファイルの差し替え**

本プロジェクトの `resources` ファイルを `coachtech-prepared-file/Preparedblade-mockcase-BookShelf` リポジトリの **Basicブランチ** の `resources` ファイルと入れ替えてください。
...
### 8. Laravel Fortify の導入（基本）

認証機能を実装するため、Laravel Fortify を導入します。
...
本プロジェクトでは2要素認証を使用しないため、以下を削除します。

- `database/migrations/2014_10_12_200000_add_two_factor_columns_to_users_table.php`

**4. `config/app.php` の `providers` 配列に追加**

```php
App\Providers\FortifyServiceProvider::class,
```

**5. `config/fortify.php` の `features` 配列を以下に変更（登録機能のみ有効化）**
...
**6. `FortifyServiceProvider` のカスタマイズ**

- Fortify 標準の views を無効化: `config/fortify.php` の `'views' => false`
- login / register の view を独自定義: `Fortify::loginView(...)`, `Fortify::registerView(...)`
- `Fortify::createUsersUsing(CreateNewUser::class)` で Action を登録
- `CreateNewUser::create()` の `Validator::make()` に `messages()` を追加し、シート7 のバリデーションメッセージを日本語で定義
- ログアウト後のリダイレクト先を `/login` に変更（カスタム `LogoutResponse` を登録）

### 9. 認証ルートの定義 (`routes/auth.php`)（基本）

Fortify の view を無効化したため、`/login`, `/register` の GET ルート、および `/logout` の POST ルートを独自に定義します。

**1. `routes/auth.php` を新規作成し、以下を定義**

- GET `/login` → `view('auth.login')`, name: login
- GET `/register` → `view('auth.register')`, name: register
- POST `/logout` → セッション破棄して `/login` にリダイレクト

**2. `app/Providers/RouteServiceProvider.php` の `boot()` 内で `routes/auth.php` を `web` ミドルウェア下で読み込むよう追加**

### 10. 日本語ロケールの設定（基本）

バリデーションメッセージを日本語化します。

**1. `config/app.php` の `locale` を `'ja'` に変更**

**2. `lang/ja/` ディレクトリを作成し、以下ファイルを配置**

- `auth.php`: `'failed' => 'ログイン情報が登録されていません。'` 等
- `validation.php`: `'required' => ':attributeを入力してください。'`, `'email' => ':attributeはメール形式で入力してください。'`, `'confirmed' => ':attributeと一致しません。'`, `'unique' => 'その:attributeは既に使用されています。'`, `attributes => ['name' => 'お名前', 'email' => 'メールアドレス', 'password' => 'パスワード']` 等
- `passwords.php`（reset用、Fortify のデフォルト構成で必要）
...
以下ファイルを本プロジェクト仕様に調整します。

**1. `resources/js/app.js` — Alpine.js を初期化**
...
**2. `resources/css/app.css` — Tailwind ディレクティブ**
...
**3. `vite.config.js` — input を設定**

```js
input: ['resources/css/app.css', 'resources/js/app.js']
```

**4. `postcss.config.js` — ES module 形式で記述**

```js
export default { plugins: { tailwindcss: {}, autoprefixer: {} } };
```

**5. `app/Exceptions/Handler.php` — API エンドポイント向け例外ハンドリング**

以下の例外を日本語の JSON レスポンスに変換するよう `render()` をオーバーライドします。

- `ModelNotFoundException` → 404 `{"error": "書籍が見つかりませんでした。"}`
- ★ 応用機能で追加: `AuthorizationException` → 403 `{"error": "この操作を実行する権限がありません。"}`

**6. `app/Providers/AuthServiceProvider.php` — Policy 登録**

`$policies` 配列に `Book::class => BookPolicy::class`, `Review::class => ReviewPolicy::class` を登録します。

### 12. データベースのマイグレーションと初期データ投入（基本）
...
### ★ 13. 応用機能用Bladeテンプレートのインポートと環境調整（応用）

基本機能の実装完了後、応用機能の画面に対応する Blade テンプレートを取得します。

**1. Bladeテンプレートの差し替え**

`coachtech-prepared-file/Preparedblade-mockcase-BookShelf` リポジトリの **Advancedブランチ** から `resources` ファイルを再度インポートし、プロジェクトの `resources` ディレクトリを置き換えてください。

**2. Sanctum 関連の追加**（応用版の公開API 書き込み系を認証必須化するため）
...
- 上記コマンドで `config/sanctum.php`, `personal_access_tokens` migration が生成される
- `User` モデルに `HasApiTokens` トレイトを追加

**3. データモデル変更の適用**

新規テーブル（reading_plans）のマイグレーションを書き、Laravel 標準の notifications テーブル用マイグレーションも生成して、再構築します。変更点の詳細はデータ要件・テーブル仕様書を参照してください。

```bash
# notifications テーブル用マイグレーションを生成（Laravel 標準）
sail artisan notifications:table

# DB を再構築（既存マイグレーション + 新規 reading_plans + notifications）
sail artisan migrate:fresh --seed
```

**4. Google Books API キーの設定**

- `.env` に `GOOGLE_BOOKS_API_KEY=...` を追加
- `config/services.php` に `'google' => ['books_api_key' => env('GOOGLE_BOOKS_API_KEY')]` を追加
- APIキーの取得手順は README を参照

**5. Sanctum ミドルウェアと Policy の適用**

- `routes/api.php` の書き込み系ルートを `auth:sanctum` ミドルウェアで保護
- `Api\V1\BookController` の `update/destroy` で `$this->authorize(...)` を呼ぶ
- `app/Providers/AuthServiceProvider.php` の `$policies` 配列に `ReadingPlan::class => ReadingPlanPolicy::class` を追加（応用機能で追加した読書計画機能の認可登録）

**6. ★ 日次バッチの Cron 登録（応用）**

読書計画のリマインダー通知 + Auto-expire を毎日 20:00 に実行するため、Schedule を Console Kernel に登録します。
...
- 本番環境ではサーバー側で以下を Cron に登録（毎分 `schedule:run` を呼び出し、Schedule に登録された 20:00 のジョブが発火する）:

```cron
* * * * * cd /var/www/html && php artisan schedule:run >> /dev/null 2>&1
```

- 開発環境では `sail artisan schedule:work` を別ターミナルで起動しておくことで、`schedule:run` が 1 分ごとに自動実行される（手動確認時は `sail artisan reading-plans:run-daily` で直接実行可能）。

> **【提出前チェック】依存パッケージの脆弱性監査（Composer Audit）**
>
> 近年、Composer パッケージを標的としたサプライチェーン攻撃（正規パッケージが改ざん・乗っ取られ、`composer install` 時に悪意あるコードが実行される事例）が報告されています。安全のため、提出前に必ず `composer audit` を実行し、依存パッケージに既知の脆弱性が含まれていないことを確認してください。
>
> **「High」または「Critical」の脆弱性が検出された場合は、再提出となります。** 検出された場合は、該当パッケージのアップデートや差し替えで脆弱性を解消してから提出してください。
>
> ※ `composer audit`（Composer 2.4 以降で利用可能）は、`composer.lock` の内容を既知の脆弱性データベース（Packagist / GitHub Advisory Database 等）と照合するコマンドです。
...
| PG03 | 書籍登録 | GET | /books/create | Blade提供済み。<br>認証必須。全ジャンル一覧をチェックボックスで表示。<br>routes/web.php<br>app/Http/Controllers/BookController@create<br>resources/views/books/create.blade.php | 基本 |
...
| ★ PG15 | ★ 読書計画一覧<br>【新規】 | GET | /reading-plans | Blade提供済み（Advancedブランチ）。<br>認証必須。ログインユーザーの読書計画を一覧表示。<br>ReadingPlanStatus Enum を活用した status 絞り込みドロップダウン（in_progress / completed / expired）を備える。<br>各行に「読了する」「編集」「削除」ボタン。<br>routes/web.php<br>app/Http/Controllers/ReadingPlanController@index<br>resources/views/reading-plans/index.blade.php | ★ 応用 |
| ★ PG16 | ★ 読書計画作成<br>【新規】 | GET | /reading-plans/create | Blade提供済み（Advancedブランチ）。<br>認証必須。書籍プルダウン（全書籍から選択）と期日入力フォームを表示。<br>routes/web.php<br>app/Http/Controllers/ReadingPlanController@create<br>resources/views/reading-plans/create.blade.php | ★ 応用 |
| ★ PG17 | ★ 読書計画編集<br>【新規】 | GET | /reading-plans/{plan}/edit | Blade提供済み（Advancedブランチ）。<br>認証+認可（ReadingPlanPolicy@update）必須。所有者のみ閲覧可。<br>期日変更フォームを表示。<br>completed 計画はアクセス時に 403 を返す。<br>routes/web.php<br>app/Http/Controllers/ReadingPlanController@edit<br>resources/views/reading-plans/edit.blade.php | ★ 応用 |
| ★ PG18 | ★ 通知一覧<br>【新規】 | GET | /notifications | Blade提供済み（Advancedブランチ）。<br>認証必須。ヘッダーベルアイコン（未読件数バッジ）から遷移。<br>通知データ（plan_id / book_title / timing / title / body）を時系列で表示。各通知に既読化ボタン。<br>routes/web.php<br>app/Http/Controllers/NotificationController@index<br>resources/views/notifications/index.blade.php | ★ 応用 |

※ 応用機能では `resources/views/layouts/navigation.blade.php` も差し替えられ、ヘッダー右側に「読書計画」リンクとベルアイコン（/notifications へのリンク + 未読件数バッジ）が追加される。
...
| | 「ログイン」ボタンを押す | GET /login | ログイン画面に遷移する | ボタン | 基本 |
...
| | 「会員登録」ボタンを押す | GET /register | 会員登録画面に遷移する | ボタン | 基本 |
...
| | 評価プルダウン選択、レビュー投稿フォームに入力して「投稿する」ボタンを押す | POST /books/{book}/reviews | 認証時→バリデーション通過後にレビューが保存され、書籍詳細画面にリダイレクトし「レビューを投稿しました。」と表示。未認証時→ /login にリダイレクト<br>バリデーション: rating(required/integer/min:1/max:5), comment(required/string/max:1000)<br>エラーメッセージ:<br>　a. rating が未入力の場合：評価は必須です<br>　b. comment が未入力の場合：コメントは必須です<br>　c. comment が1000文字超過の場合：コメントは1000文字以内で入力してください | ReviewController@store, StoreReviewRequest, $book->reviews()->create([...]) | 基本 |
...
| **書籍登録画面** | ページを表示する | GET /books/create | 認証時→タイトル・著者・ISBN・出版日・説明・画像URLの入力欄と、全ジャンル一覧のチェックボックス（複数選択可）が表示される（応用時は「ISBN書籍情報自動入力フォーム」も含む）。未認証時→ /login にリダイレクト | BookController@create, auth middleware, Genre::all(), books.create | 基本 |
| | 全項目を入力して「登録」ボタンを押す | POST /books | バリデーション通過時→書籍がDBに登録され、book_genreテーブルにジャンル紐付けが作成され、書籍詳細画面にリダイレクトし「書籍を登録しました。」と表示。失敗時→フォームに戻りエラー表示<br>バリデーション: title(required/string/max:255), author(required/string/max:255), isbn(required/string/size:13/unique:books,isbn), published_date(required/date), description(nullable/string), image_url(nullable/url), genres(required/array), genres.*(exists:genres,id)<br>エラーメッセージ：<br>a. 入力必須項目未入力の場合：詳細はシート7参照<br>b. ISBNが13桁でない場合：ISBNは13桁で入力してください。<br>c. ISBNが重複している場合：そのISBNは既に使用されています。<br>d. 画像URLがURL形式でない場合：画像URLは有効なURL形式で入力してください。<br>e. タイトルが255文字を超えた場合：タイトルは255文字以内で入力してください。<br>f. 著者名が255文字を超えた場合：著者名は255文字以内で入力してください。<br>g. 出版日が有効な日付形式でない場合：出版日は有効な日付形式で入力してください。<br>h. ジャンルが未設定の場合：ジャンルは1つ以上選択してください。 | BookController@store, StoreBookRequest, $request->user()->books()->create($data), $book->genres()->attach($genres) | 基本 |
| | ★ ISBN（13桁）を入力して「ISBN検索」ボタンを押す | GET /books/isbn/{isbn} | 13桁ISBN→Google Books API から書籍情報を取得し、JavaScriptで title/author/published_date/description/image_url 入力欄に自動入力される<br>13桁以外→400「ISBNは13桁で入力してください。」<br>書籍見つからず→404「書籍が見つかりませんでした。」<br>Google Books API のクォータ超過→429「Google Books API のクォータを超過しました。.env に GOOGLE_BOOKS_API_KEY を設定してください。」<br>API通信エラー→500「API通信エラーが発生しました。」 | BookController@searchByIsbn, Http::get('https://www.googleapis.com/books/v1/volumes?q=isbn:{isbn}&key={API_KEY}'), config/services.php の google.books_api_key | ★ 応用 |
...
| | 内容を変更して「更新」ボタンを押す | PUT /reviews/{review} | 認証＋投稿者本人＋バリデーション通過→レビューが更新され、書籍詳細画面にリダイレクトし「レビューを更新しました。」と表示。他ユーザー→403 Forbidden（サーバー側で実装されていること）。<br>バリデーション: rating(required/integer/min:1/max:5), comment(required/string/max:1000)<br>エラーメッセージ:<br>　a. rating が未入力の場合：評価は必須です<br>　b. comment が未入力の場合：コメントは必須です<br>　c. comment が1000文字超過の場合：コメントは1000文字以内で入力してください | ReviewController@update, UpdateReviewRequest, $review->update([...]) | 基本 |
...
| **★ 読書計画一覧画面** | ★ ページを表示する | GET /reading-plans | 認証時→ログインユーザーの計画を一覧表示し、ReadingPlanStatus Enum を活用した status 絞り込みドロップダウン（先頭に「すべて」、続けて in_progress / completed / expired）を備える。未選択時は全件、選択時は対応する Eloquent scope で絞り込む。各行に書籍タイトル・期日・完了日（completed のみ表示、それ以外は空欄）・状態バッジと「読了する」「編集」「削除」ボタン。target_date 昇順で並ぶ。未認証時→ /login にリダイレクト | ReadingPlanController@index, auth middleware, Auth::user()->readingPlans()->orderBy('target_date'), Eloquent scope（active/completed/expired）, reading-plans.index<br>routes/web.php では `Route::resource('reading-plans', ReadingPlanController::class)->except(['show'])` で 6 アクション（index/create/store/edit/update/destroy）を定義し、`POST /reading-plans/{plan}/complete` のみ別ルート（reading-plans.complete）として定義する | ★ 応用 |
| | ★ ステータス絞り込みドロップダウンを変更する | GET /reading-plans?status=xxx | 選択した status の計画のみ表示される | クエリパラメータ + Eloquent scope | ★ 応用 |
| | ★ 「読了する」ボタンを押す | POST /reading-plans/{plan}/complete | 認証＋所有者→status=Completed に更新 + completed_at=now() セット + 紐づく Notification は影響なし、一覧にリダイレクトしフラッシュメッセージ「読書計画を完了しました。」と表示。他ユーザー→403 Forbidden | ReadingPlanController@complete, ReadingPlanPolicy@update, $plan->update(['status' => ReadingPlanStatus::Completed, 'completed_at' => now()]) | ★ 応用 |
| | ★ 「編集」ボタンを押す | GET /reading-plans/{plan}/edit | 認証＋所有者かつ status !== Completed→読書計画編集画面に遷移。他ユーザー→403 Forbidden、completed の場合→403 Forbidden | ReadingPlanController@edit, ReadingPlanPolicy@update, UpdateReadingPlanRequest::authorize() | ★ 応用 |
| | ★ 「削除」ボタンを押す | DELETE /reading-plans/{plan} | 認証＋所有者→Transaction 内で関連リマインダー通知も同時削除し計画を削除（notifications は Polymorphic のため CASCADE が効かないので手動削除）、一覧にリダイレクトしフラッシュメッセージ「読書計画を削除しました。」と表示。他ユーザー→403 Forbidden | ReadingPlanController@destroy, ReadingPlanPolicy@delete, DB::transaction（関連通知削除 + 計画削除）, Auth::user()->notifications()->where('data->plan_id', $plan->id)->delete() | ★ 応用 |
| | ★ 「新規計画作成」ボタンを押す | GET /reading-plans/create | 認証時→読書計画作成画面に遷移 | リンク | ★ 応用 |
| **★ 読書計画作成画面** | ★ ページを表示する | GET /reading-plans/create | 認証時→書籍プルダウン（全書籍から選択、タイトル昇順）と期日入力フォームが表示される。未認証時→ /login にリダイレクト | ReadingPlanController@create, auth middleware, Book::orderBy('title')->get(), reading-plans.create | ★ 応用 |
| | ★ 書籍と期日を入力して「登録」ボタンを押す | POST /reading-plans | バリデーション通過時→計画がDBに登録され、status='in_progress' がセットされ、一覧にリダイレクトしフラッシュメッセージ「読書計画を作成しました。」と表示。失敗時→フォームに戻りエラー表示<br>バリデーション: book_id(required/integer/exists:books,id), target_date(required/date/after_or_equal:today), 重複制御（同一 user × book で in_progress 状態の他計画が存在しないこと）<br>エラーメッセージ: 詳細はシート7参照 | ReadingPlanController@store, StoreReadingPlanRequest, Auth::user()->readingPlans()->create(['book_id' => ..., 'target_date' => ..., 'status' => ReadingPlanStatus::InProgress]) | ★ 応用 |
| **★ 読書計画編集画面** | ★ ページを表示する | GET /reading-plans/{plan}/edit | 認証＋所有者かつ status !== Completed→既存の target_date が初期値として表示された編集フォームが表示される。他ユーザー・completed→403 Forbidden | ReadingPlanController@edit, ReadingPlanPolicy@update, reading-plans.edit | ★ 応用 |
| | ★ 期日を変更して「更新」ボタンを押す | PUT /reading-plans/{plan} | バリデーション通過時→target_date 更新、Expired 計画の場合は status を in_progress に復帰、一覧にリダイレクトしフラッシュメッセージ「読書計画を更新しました。」と表示。他ユーザー・completed→403 Forbidden<br>バリデーション: target_date(required/date/after_or_equal:today), 重複制御（自身を除外）<br>エラーメッセージ: 詳細はシート7参照 | ReadingPlanController@update, UpdateReadingPlanRequest, $plan->update([...]) | ★ 応用 |
| **★ 通知一覧画面** | ★ ヘッダーベルアイコンをクリック | GET /notifications | 認証時→ログインユーザーの通知一覧（unreadNotifications + read 通知）を時系列降順で表示。各通知に通知データ（plan_id / book_title / timing / title / body）と「既読にする」ボタン | NotificationController@index, auth middleware, Auth::user()->notifications, notifications.index | ★ 応用 |
| | ★ 「既読にする」ボタンを押す | POST /notifications/{id}/read | 認証＋所有者→当該通知の read_at が now() でセットされ、通知一覧にリダイレクトしフラッシュメッセージ「通知を既読にしました。」と表示。ヘッダーベルアイコンの未読件数バッジが更新される | NotificationController@markAsRead, $notification->markAsRead() | ★ 応用 |
| **★ 日次バッチ処理** | ★ 毎日 20:00 に Schedule で実行される | `php artisan schedule:run`（Cron で 1 分毎に実行され、Schedule に登録された 20:00 のジョブを発火） | 1. 期日経過した in_progress 計画を一括 Expired 化（mass update）<br>2. 期日 3 日前の in_progress 計画に PlanReminderNotification（予告）を DatabaseChannel で発火<br>3. 期日当日の in_progress 計画に PlanReminderNotification（最終リマインド）を DatabaseChannel で発火<br>4. 期日 3 日後の Expired 計画に PlanReminderNotification（再エンゲージメント）を DatabaseChannel で発火<br><br>Schedule で daily 実行が保証されるため、同日中の重複発火は構造上発生しない。 | Console Kernel `$schedule->command(...)->daily()->at('20:00')`, Console Command `app/Console/Commands/RunReadingPlanDailyBatch.php`, ReadingPlan::query(), Notification facade（DatabaseChannel）, PlanReminderNotification（timing パラメータで予告/最終リマインド/再エンゲージメントを切替） | ★ 応用 |

### ★ リマインダー通知文言（応用）

PlanReminderNotification の `toDatabase()` で生成し、通知一覧画面に表示される `title` / `body` の文言は以下のとおり。

| タイミング | title | body |
|---|---|---|
| 期限 3 日前（予告） | 読書計画リマインド — 期限まであと 3 日 | 「{書籍タイトル}」の期限まで残り 3 日です。引き続き読書を進めましょう。 |
| 期限当日（最終リマインド） | 読書計画 — 本日が期限 | 「{書籍タイトル}」は本日が期限です。読了済みなら完了登録を、もう少し必要なら期限を変更してください。 |
| 期限 3 日後（再エンゲージメント） | 読書計画 — 期限超過 3 日経過 | 「{書籍タイトル}」の期限から 3 日が経過しました。読了済みなら完了登録、続けるなら期限を変更してください。 |
...
| | comment | required / string / max:1000 | 基本 |
...
| ★ 書籍登録<br>(StoreBookRequest) ※ Web版 | published_date | ★ nullable / date（required→nullable変更） | ★ 応用 |
| ★ 書籍編集<br>(UpdateBookRequest) ※ Web版 | published_date | ★ nullable / date（required→nullable変更） | ★ 応用 |
...
| ★ 計画作成<br>(StoreReadingPlanRequest) | book_id | ★ required / integer / exists:books,id | ★ 応用 |
| | target_date | ★ required / date / after_or_equal:today | ★ 応用 |
| | （重複制御） | ★ Custom rule: 同一 user × book で in_progress 状態の他計画が存在しないこと | ★ 応用 |
| ★ 計画編集<br>(UpdateReadingPlanRequest) | target_date | ★ required / date / after_or_equal:today | ★ 応用 |
| | （重複制御） | ★ Custom rule: 同一 user × book で in_progress 状態の他計画が存在しないこと（自身を除外） | ★ 応用 |
| | （状態制限） | ★ 認可は ReadingPlanPolicy@update に集約（「所有者 かつ completed でない」を判定）。Controller で `$this->authorize('update', $plan)` を呼ぶ。completed 計画は編集不可（403 を返す） | ★ 応用 |
...
| FormRequest | メッセージ定義 |
|---|---|
| StoreBookRequest<br>UpdateBookRequest<br>（Web版） | title.required: タイトルは必須です。<br>title.string: タイトルは文字列で入力してください。<br>title.max: タイトルは255文字以内で入力してください。<br>author.required: 著者名は必須です。<br>author.string: 著者名は文字列で入力してください。<br>author.max: 著者名は255文字以内で入力してください。<br>isbn.required: ISBNは必須です。<br>isbn.string: ISBNは文字列で入力してください。<br>isbn.size: ISBNは13桁で入力してください。<br>isbn.unique: そのISBNは既に使用されています。<br>published_date.required: 出版日は必須です。<br>published_date.date: 出版日は有効な日付形式で入力してください。<br>description.string: 説明は文字列で入力してください。<br>image_url.url: 画像URLは有効なURL形式で入力してください。<br>image_url.max: 画像URLは255文字以内で入力してください。<br>genres.required: ジャンルは1つ以上選択してください。<br>genres.array: ジャンルは配列で入力してください。<br>genres.*.exists: 選択されたジャンルは存在しません。 |
| Api\V1\StoreBookRequest<br>Api\V1\UpdateBookRequest<br>（API版・Basic 以降） | Web版と概ね同等。応用では Web版が published_date を nullable にするのに対し API版は required を維持するため、published_date.required: 出版日は必須です。を追加で定義（isbn は Web版・API版とも required のため isbn.required は共通で定義）。 |
| StoreReviewRequest<br>UpdateReviewRequest | rating.required: 評価は必須です。<br>rating.integer: 評価は整数で入力してください。<br>rating.min: 評価は1〜5の整数で入力してください。<br>rating.max: 評価は1〜5の整数で入力してください。<br>comment.required: コメントは必須です。<br>comment.string: コメントは文字列で入力してください。<br>comment.max: コメントは1000文字以内で入力してください。 |
...
| ★ StoreReadingPlanRequest<br>UpdateReadingPlanRequest | book_id.required: 書籍を選択してください。<br>book_id.integer: 書籍IDは整数で入力してください。<br>book_id.exists: 選択された書籍は存在しません。<br>target_date.required: 期日は必須です。<br>target_date.date: 期日は有効な日付形式で入力してください。<br>target_date.after_or_equal: 期日は今日以降の日付を指定してください。<br>（重複制御）: この書籍は既に進行中の読書計画が存在します。 |
...
| ReviewSeeder | reviews テーブルにレビューデータを32件投入する。<br><br>5人のユーザーが11冊の書籍に対してレビューを投稿。<br>rating は 3〜5 の範囲。<br>各書籍に2〜4件のレビューを配分。<br>具体的なコメント内容を設定すること。<br><br>create を使用。 | 基本 |
...
| ★ ReadingPlanSeeder | reading_plans テーブルに読書計画データを 6 件投入する。<br>採点者がいつ実行しても同じ挙動になるよう、`Carbon::today()` 起点で動的に target_date を設定する。<br>動作確認の効率を考えて、主要シナリオは 1 ユーザーに集約する。<br><br>**山田太郎（主要シナリオ集約、ID 1〜5）**:<br>1. target_date = `Carbon::today()->addDays(3)` / status = 'in_progress' → 3日前リマインダー対象<br>2. target_date = `Carbon::today()` / status = 'in_progress' → 当日リマインダー対象<br>3. target_date = `Carbon::today()->subDays(3)` / status = 'in_progress' → バッチで Auto-expire 化 + 3日後再エンゲージメント対象（二重シナリオ）<br>4. target_date = `Carbon::today()->addDays(7)` / status = 'in_progress' → リマインダー対象外<br>5. target_date = `Carbon::today()->subDays(10)` / status = 'completed' / completed_at = `Carbon::today()->subDays(5)` → 完了済み<br><br>**鈴木花子（他ユーザー認可テスト用、ID 6）**:<br>6. target_date = `Carbon::today()->addDays(5)` / status = 'in_progress' → 山田太郎ログイン中に URL `/reading-plans/6/edit` 直打ちで 403 確認<br><br>book_id は計画ごとに異なる書籍を割り当てる（同一ユーザー in_progress 重複制御に抵触しないよう）。<br>create を使用。 | ★ 応用 |
| ★ DatabaseSeeder | 末尾で ReadingPlanSeeder を追加で呼び出す。<br><br>実行順（応用）:<br>1. UserSeeder<br>2. GenreSeeder<br>3. BookSeeder<br>4. ReviewSeeder<br>5. FavoriteSeeder<br>6. ReviewLikeSeeder<br>7. ReadingPlanSeeder（応用） | ★ 応用 |
...
| | ★ 書籍CRUD | ★ 書籍編集認可 | BookPolicy による認可テスト。所有者は can('update', $book) が true、他ユーザーは403 Forbidden。書籍編集画面（GET /books/{book}/edit）にアクセスし、View に正しい book と genres が渡されていること（assertViewHas / viewData で検証）。 | ★ 応用 |
...
| | ★ 読書計画 | ★ 計画 CRUD・Policy・「読了」・期限変更 | 認証ユーザーが計画を作成・編集・削除でき、reading_plans テーブルにレコードが作成・更新・削除されること。所有者のみが編集・削除でき、他ユーザーは403 Forbiddenとなること。「読了する」操作で status が Completed に更新され completed_at がセットされること。期限変更時に Expired 計画は in_progress に復帰すること。completed 計画の編集アクセスで 403 Forbidden が返ること。計画削除時に関連するリマインダー通知も同時に削除されること。 | ★ 応用 |
| | ★ リマインダーバッチ | ★ 3 タイミング発火 | Carbon::setTestNow による時刻固定で、期日 3 日前 / 当日 / 3 日後 の 3 タイミングで PlanReminderNotification が DatabaseChannel に発火すること（notifications テーブルに対応行が作成されることを assert）。リマインダー対象外の計画には発火しないこと。 | ★ 応用 |
| | ★ Auto-expire バッチ | ★ 一括 Expired 化 | 期日経過した in_progress 計画が Expired に一括更新されること。期日経過していない計画は in_progress のままであること。 | ★ 応用 |
...
| DR05 | お気に入り | ユーザーのお気に入り書籍 | id（サロゲートキー）、user_id, book_id（複合 unique 制約） | favorites テーブル。<br>「お気に入り」というエンティティとして扱うため、id をサロゲートキー、(user_id, book_id) に複合 unique 制約。 | 基本 |
| DR06 | いいね | レビューへのいいね | id（サロゲートキー）、user_id, review_id（複合 unique 制約） | review_likes テーブル。<br>「いいね」というエンティティとして扱うため、id をサロゲートキー、(user_id, review_id) に複合 unique 制約。 | 基本 |
...
| DR01 | ★ 書籍情報 | 【変更】 | published_date: NOT NULL → nullable に変更 | | ★ 応用 |
| DR09 | ★ 読書計画 | ユーザーが書籍ごとに「いつまでに読むか」を計画するレコード | user_id（外部キー、cascade delete）<br>book_id（外部キー、cascade delete）<br>target_date（date、必須、今日以降）<br>status（enum: in_progress / completed / expired）<br>completed_at（timestamp、nullable） | reading_plans テーブル。<br>app/Models/ReadingPlan | ★ 応用 |
| DR10 | ★ 通知 | 読書計画リマインダーの通知レコード | id（UUID）<br>type（Notification クラス名）<br>notifiable_type / notifiable_id（Polymorphic）<br>data（JSON: plan_id / book_title / timing / title / body）<br>read_at（既読日時、nullable） | notifications テーブル。<br>`php artisan notifications:table` で生成。Laravel標準。 | ★ 応用 |
...
| | | user_id | bigint unsigned | | ○ | users.id | ON DELETE CASCADE |
| | | book_id | bigint unsigned | | ○ | books.id | ON DELETE CASCADE |
| | | | | | | | 複合 unique 制約: (user_id, book_id) |
| 7 | review_likesテーブル | | | | | | |
| | | id | bigint unsigned | ○ | ○ | | $table->id() |
| | | user_id | bigint unsigned | | ○ | users.id | ON DELETE CASCADE |
| | | review_id | bigint unsigned | | ○ | reviews.id | ON DELETE CASCADE |
| | | | | | | | 複合 unique 制約: (user_id, review_id) |
| 8 | reading_plansテーブル | | | | | | | ★ 応用機能で追加 |
| | | id | bigint unsigned | ○ | ○ | | $table->id() |
| | | user_id | bigint unsigned | | ○ | users.id | ON DELETE CASCADE<br>計画者 |
| | | book_id | bigint unsigned | | ○ | books.id | ON DELETE CASCADE<br>対象書籍 |
| | | target_date | date | | ○ | | 期日（今日以降のみバリデーション） |
| | | status | varchar(20) | | ○ | | 'in_progress' / 'completed' / 'expired'<br>ReadingPlanStatus PHP Enum で cast<br>label() 戻り値: 進行中 / 完了 / 期限切れ<br>デフォルト: 'in_progress' |
| | | completed_at | timestamp | | | | NULL許可（nullable）<br>「読了する」操作時にセット |
| | | created_at | timestamp | | | | $table->timestamps() |
| | | updated_at | timestamp | | | | $table->timestamps() |
| | | | | | | | INDEX: (user_id, status) / target_date |
| 9 | notificationsテーブル | | | | | | | ★ 応用機能で追加<br>Laravel標準（`php artisan notifications:table` で生成） |
| | | id | char(36) | ○ | ○ | | UUID（$table->uuid('id')->primary()） |
| | | type | varchar(255) | | ○ | | Notification クラス名（例: PlanReminderNotification） |
| | | notifiable_type | varchar(255) | | ○ | | Polymorphic（通常 'App\Models\User'） |
| | | notifiable_id | bigint unsigned | | ○ | | Polymorphic |
| | | data | text | | ○ | | JSON: plan_id / book_title / timing / title / body |
| | | read_at | timestamp | | | | NULL許可（NULL = 未読） |
| | | created_at | timestamp | | | | $table->timestamps() |
| | | updated_at | timestamp | | | | $table->timestamps() |
...
erDiagram
    ...
    book_genre {
        bigint_unsigned book_id PK_FK
        bigint_unsigned genre_id PK_FK
    }
    ...
    review_likes {
        bigint_unsigned user_id PK_FK
        bigint_unsigned review_id PK_FK
    }
    ...
    users ||--o{ reading_plans : "plans (1対多) ★応用"
    books ||--o{ reading_plans : "planned (1対多) ★応用"
    users ||--o{ notifications : "receives (1対多 / Polymorphic) ★応用"
```
...
| 認証 | 不要 |
| 認可（ポリシー） | - |
| 説明 | 書籍一覧を取得する。検索・絞り込み・ページネーション対応 |
