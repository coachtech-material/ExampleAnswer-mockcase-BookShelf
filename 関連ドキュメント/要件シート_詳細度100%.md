...
| **ランキング画面** | ページを表示する | GET /ranking | レビュー平均評価のTOP10書籍が降順で表示される。レビューがない書籍は表示されない。ゲストもアクセス可能 | RankingController@index, $rankedBooks = Book::withAvg('reviews', 'rating')->withCount('reviews')->has('reviews')->orderByDesc('reviews_avg_rating')->take(10)->get(), ranking.index | 基本 |
