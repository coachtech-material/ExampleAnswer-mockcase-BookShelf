...
| ★ 読書計画機能の設計: 計画CRUD・状態遷移・期限変更 | 「読書計画は1書籍1計画で、書籍プルダウンから選択する設計にしましたが、この方針で問題ないですか？」<br>「進捗管理は『読了する』ボタンのみで、% 入力は不要にする設計にしましたが、この方針で合っていますか？」<br>「同一書籍に in_progress 状態の他計画があれば作成不可、completed 後は再計画可とする設計にしましたが、この制約で問題ないですか？」<br>「期日変更時に last_notified_at を NULL リセット、Expired 計画は in_progress に復帰、completed 計画は編集不可とする設計にしましたが、この挙動で合っていますか？」 | 機能要件 — 読書計画一覧/作成/編集画面の各操作行 + バリデーションルール — StoreReadingPlanRequest / UpdateReadingPlanRequest | ★ 応用 |
| ★ リマインダー通知の設計: 発火タイミング・条件・チャネル | 「リマインダーは期日 3 日前 / 当日 / 3 日後 の 3 タイミングで毎日 20:00 に発火する設計にしましたが、この方針で問題ないですか？」<br>「3 日後の通知は status=Expired の計画に対する『再エンゲージメント』通知として発火する設計にしましたが、この役割設計で合っていますか？」<br>「last_notified_at カラムで同日重複防止する設計にしましたが、この方式で問題ないですか？」<br>「通知は DatabaseChannel のみ採用し、メール送信はしない設計にしましたが、このチャネル選択で合っていますか？」<br>「読了完了時の通知は発火せず、フラッシュメッセージのみで完結する設計にしましたが、この方針で問題ないですか？」 | 機能要件 — 日次バッチ処理の行 + シート3 の Schedule 登録手順 | ★ 応用 |
...
| | ★ Database Transaction | ・複数行を一括更新する処理（Auto-expire の Expired 化）や、複数フィールドを連動更新する処理（「読了する」操作で status + completed_at セット、期限変更で Expired→in_progress 復帰時の status + completed_at + last_notified_at リセット）は `DB::transaction()` で原子化する | 中途半端な状態を防ぐ | ★ 応用 |
...
| ★ PG17 | ★ 読書計画編集<br>【新規】 | GET | /reading-plans/{plan}/edit | Blade提供済み（Advancedブランチ）。<br>認証+認可（ReadingPlanPolicy@update）必須。所有者のみ閲覧可。<br>期日変更フォームを表示。Expired 計画の編集時は「期日を変更すると進行中に戻ります」の注意書きを表示。<br>completed 計画はアクセス時に 403 を返す。<br>routes/web.php<br>app/Http/Controllers/ReadingPlanController@edit<br>resources/views/reading-plans/edit.blade.php | ★ 応用 |
...
| **★ 読書計画一覧画面** | ★ ページを表示する | GET /reading-plans | 認証時→ログインユーザーの計画を一覧表示し、ReadingPlanStatus Enum を活用した status 絞り込みドロップダウン（in_progress / completed / expired）を備える。各行に「読了する」「編集」「削除」ボタン。未認証時→ /login にリダイレクト | ReadingPlanController@index, auth middleware, Auth::user()->readingPlans()->where('status', $status), Eloquent scope（active/completed/expired）, reading-plans.index | ★ 応用 |
| | ★ ステータス絞り込みドロップダウンを変更する | GET /reading-plans?status=xxx | 選択した status の計画のみ表示される | クエリパラメータ + Eloquent scope | ★ 応用 |
| | ★ 「読了する」ボタンを押す | POST /reading-plans/{plan}/complete | 認証＋所有者→Transaction 内で status=Completed に更新 + completed_at=now() セット + 紐づく Notification は影響なし、一覧にリダイレクトしフラッシュメッセージ「読書計画を完了しました。」と表示。他ユーザー→403 Forbidden | ReadingPlanController@complete, ReadingPlanPolicy@update, DB::transaction, $plan->update(['status' => ReadingPlanStatus::Completed, 'completed_at' => now()]) | ★ 応用 |
...
| | ★ 「削除」ボタンを押す | DELETE /reading-plans/{plan} | 認証＋所有者→計画削除後、一覧にリダイレクトしフラッシュメッセージ「読書計画を削除しました。」と表示。関連 notifications は ON DELETE CASCADE 対象外（Polymorphic のため、必要に応じて手動削除）。他ユーザー→403 Forbidden | ReadingPlanController@destroy, ReadingPlanPolicy@delete | ★ 応用 |
| | ★ 「新規計画作成」ボタンを押す | GET /reading-plans/create | 認証時→読書計画作成画面に遷移 | リンク | ★ 応用 |
| **★ 読書計画作成画面** | ★ ページを表示する | GET /reading-plans/create | 認証時→書籍プルダウン（全書籍から選択）と期日入力フォームが表示される。未認証時→ /login にリダイレクト | ReadingPlanController@create, auth middleware, Book::all(), reading-plans.create | ★ 応用 |
| | ★ 書籍と期日を入力して「登録」ボタンを押す | POST /reading-plans | バリデーション通過時→計画がDBに登録され、status='in_progress'、started_at=now() がセットされ、一覧にリダイレクトしフラッシュメッセージ「読書計画を作成しました。」と表示。失敗時→フォームに戻りエラー表示<br>バリデーション: book_id(required/integer/exists:books,id), target_date(required/date/after_or_equal:today), 重複制御（同一 user × book で in_progress 状態の他計画が存在しないこと）<br>エラーメッセージ: 詳細はシート7参照 | ReadingPlanController@store, StoreReadingPlanRequest, Auth::user()->readingPlans()->create(['book_id' => ..., 'target_date' => ..., 'status' => ReadingPlanStatus::InProgress, 'started_at' => now()]) | ★ 応用 |
| **★ 読書計画編集画面** | ★ ページを表示する | GET /reading-plans/{plan}/edit | 認証＋所有者かつ status !== Completed→既存の target_date が初期値として表示された編集フォームが表示される。Expired 計画の場合は「期日を変更すると進行中に戻ります」の注意書き表示。他ユーザー・completed→403 Forbidden | ReadingPlanController@edit, ReadingPlanPolicy@update, reading-plans.edit | ★ 応用 |
| | ★ 期日を変更して「更新」ボタンを押す | PUT /reading-plans/{plan} | バリデーション通過時→target_date 更新 + last_notified_at が NULL リセット、Expired 計画の場合は status を in_progress に復帰 + completed_at を NULL リセット、一覧にリダイレクトしフラッシュメッセージ「読書計画を更新しました。」と表示。他ユーザー・completed→403 Forbidden<br>バリデーション: target_date(required/date/after_or_equal:today), 重複制御（自身を除外）<br>エラーメッセージ: 詳細はシート7参照 | ReadingPlanController@update, UpdateReadingPlanRequest, $plan->update([...]), Expired 復帰時は status と completed_at の整合性を保つため Transaction 推奨 | ★ 応用 |
...
| | ★ 「既読にする」ボタンを押す | POST /notifications/{id}/read | 認証＋所有者→当該通知の read_at が now() でセットされ、通知一覧にリダイレクトしヘッダーベルアイコンの未読件数バッジが更新される | NotificationController@markAsRead, $notification->markAsRead() | ★ 応用 |
| **★ 日次バッチ処理** | ★ 毎日 20:00 に Schedule で実行される | `php artisan schedule:run`（Cron で 1 分毎に実行され、Schedule に登録された 20:00 のジョブを発火） | 1. 期日経過した in_progress 計画を一括 Expired 化（Transaction で原子化）<br>2. 期日 3 日前の in_progress 計画に PlanReminderNotification（予告）を DatabaseChannel で発火 + last_notified_at 更新<br>3. 期日当日の in_progress 計画に PlanReminderNotification（最終リマインド）を DatabaseChannel で発火 + last_notified_at 更新<br>4. 期日 3 日後の Expired 計画に PlanReminderNotification（再エンゲージメント）を DatabaseChannel で発火 + last_notified_at 更新<br><br>last_notified_at により同日重複防止。 | Console Kernel `$schedule->command(...)->daily()->at('20:00')`, Console Command `app/Console/Commands/RunReadingPlanDailyBatch.php`, ReadingPlan::query(), Notification facade（DatabaseChannel）, PlanReminderNotification（timing パラメータで予告/最終リマインド/再エンゲージメントを切替）, DB::transaction | ★ 応用 |
...
| | （状態制限） | ★ authorize() で `$this->reading_plan->status !== ReadingPlanStatus::Completed` を返す（completed 計画は編集不可、403 を返す） | ★ 応用 |
...
| ★ ReadingPlanSeeder | reading_plans テーブルに読書計画データを 5 件投入する。<br>採点者がいつ実行しても同じ挙動になるよう、`Carbon::today()` 起点で動的に target_date を設定する。<br><br>1. 山田太郎: target_date = `Carbon::today()->addDays(3)` / status = 'in_progress' → 3日前リマインダー対象<br>2. 鈴木花子: target_date = `Carbon::today()` / status = 'in_progress' → 当日リマインダー対象<br>3. 田中一郎: target_date = `Carbon::today()->subDays(3)` / status = 'expired' / completed_at = null → 3日後再エンゲージメント対象<br>4. 佐藤美咲: target_date = `Carbon::today()->addDays(7)` / status = 'in_progress' → リマインダー対象外<br>5. 高橋健太: target_date = `Carbon::today()->subDays(10)` / status = 'completed' / completed_at = `Carbon::today()->subDays(5)` → 完了済み<br><br>book_id は BookSeeder で投入された書籍からランダム選択。started_at は計画作成時の `now()` をセット。last_notified_at は全件 null。<br>create を使用。 | ★ 応用 |
...
| | ★ 読書計画 | ★ 計画 CRUD・Policy・「読了」 | 認証ユーザーが計画を作成・編集・削除でき、reading_plans テーブルにレコードが作成・更新・削除されること。ReadingPlanPolicy により所有者のみが編集・削除でき、他ユーザーは403 Forbiddenとなること。「読了する」操作で status が Completed に更新され completed_at がセットされ、Transaction で原子化されること。 | ★ 応用 |
| | ★ 読書計画 | ★ 期限変更 | 期限変更時に last_notified_at が NULL リセットされること。Expired 計画の編集で in_progress に復帰 + completed_at が NULL リセットされること。completed 計画の編集アクセスで 403 Forbidden が返ること。 | ★ 応用 |
| | ★ リマインダーバッチ | ★ 3 タイミング発火 | Carbon::setTestNow による時刻固定で、期日 3 日前 / 当日 / 3 日後 の 3 タイミングで PlanReminderNotification が DatabaseChannel に発火すること（notifications テーブルに対応行が作成されることを assert）。リマインダー対象外の計画には発火しないこと。last_notified_at が更新されること。 | ★ 応用 |
| | ★ Auto-expire バッチ | ★ 一括 Expired 化 + Transaction | 期日経過した in_progress 計画が Expired に一括更新されること。Transaction で原子化されており、途中で例外が発生した場合にロールバックされること（DB::transaction 内で例外を発生させ、変更が巻き戻ることを assert）。 | ★ 応用 |
...
| DR09 | ★ 読書計画 | ユーザーが書籍ごとに「いつまでに読むか」を計画するレコード | user_id（外部キー、cascade delete）<br>book_id（外部キー、cascade delete）<br>target_date（date、必須、今日以降）<br>status（enum: in_progress / completed / expired）<br>started_at（timestamp、nullable）<br>completed_at（timestamp、nullable）<br>last_notified_at（timestamp、nullable、リマインダー重複防止用） | reading_plans テーブル。<br>app/Models/ReadingPlan | ★ 応用 |
...
| | | status | varchar(20) | | ○ | | 'in_progress' / 'completed' / 'expired'<br>ReadingPlanStatus PHP Enum で cast<br>デフォルト: 'in_progress' |
| | | started_at | timestamp | | | | NULL許可（nullable）<br>計画作成時にセット |
| | | completed_at | timestamp | | | | NULL許可（nullable）<br>「読了する」操作時にセット |
| | | last_notified_at | timestamp | | | | NULL許可（nullable）<br>リマインダー通知の最終発火日（同日重複防止） |
| | | created_at | timestamp | | | | $table->timestamps() |
| | | updated_at | timestamp | | | | $table->timestamps() |
...
erDiagram
    ...
    reading_plans {
        ...
        timestamp last_notified_at "nullable"
        timestamp created_at
        timestamp updated_at
    }
