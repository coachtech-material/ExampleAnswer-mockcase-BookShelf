...
##### 機能概要
ユーザーが自身で投稿したレビューを編集する機能。

##### 画面遷移
1.  **[開始]** ユーザーが自身で投稿したレビューの「編集」ボタンをクリックする。
2.  レビュー編集画面 (`/reviews/{review}/edit`) に遷移する。フォームには既存の情報が入力されている。
3.  ユーザーが必要事項を修正し、「更新」ボタンをクリックする。
4.  **[正常系]** レビュー情報がデータベースで更新され、書籍詳細画面 (`/books/{book_id}`) にリダイレクトされる。画面上部に「レビューを更新しました。」というフラッシュメッセージが表示される。
5.  **[認可エラー]** 他のユーザーが投稿したレビューの編集ページにアクセスしようとすると、403 Forbiddenエラーページが表示される。

#### 3.2.3. レビュー削除機能

##### 機能概要
ユーザーが自身で投稿したレビューを削除する機能。

##### 画面遷移
1.  **[開始]** ユーザーが自身で投稿したレビューの「削除」ボタンをクリックする。
2.  **[正常系]** レビューがデータベースから削除され、書籍詳細画面 (`/books/{book_id}`) にリダイレクトされる。画面上部に「レビューを削除しました。」というフラッシュメッセージが表示される。
3.  **[認可エラー]** 他のユーザーが投稿したレビューを削除しようとすると、403 Forbiddenエラーページが表示される。
...
##### 機能概要
ユーザーが既存のジャンル名を編集する機能。

##### 画面遷移
1.  **[開始]** ジャンル管理画面 (`/genres`) で「編集」ボタンをクリックする。
2.  ジャンル編集画面 (`/genres/{genre}/edit`) に遷移する。フォームには既存の情報が入力されている。
3.  ジャンル名を修正し、「更新」ボタンをクリックする。
4.  **[正常系]** ジャンル情報がデータベースで更新され、ジャンル管理画面 (`/genres`) にリダイレクトされる。画面上部に「ジャンルを更新しました。」というフラッシュメッセージが表示される。
5.  **[異常系]** バリデーションエラーが発生した場合、ジャンル編集画面 (`/genres/{genre}/edit`) にリダイレクトされる。入力内容は維持され、項目の下にエラーメッセージが表示される。

##### バリデーションルール（更新時）
| 項目名 | ルール | エラーメッセージ（例） |
|---|---|---|
| ジャンル名 | `Rule::unique('genres')->ignore($this->genre)` | 「そのジャンル名は既に使用されています。」 |
...
##### 機能概要
ユーザーが既存のジャンルを削除する機能。ただし、そのジャンルに紐付く書籍が存在しない場合に限る。
...
##### 機能概要
ユーザーが書籍をお気に入りに登録、またはお気に入りから解除する機能。

##### 画面遷移
1.  **[開始]** ユーザーが書籍詳細ページまたは書籍一覧ページで「お気に入り」ボタンをクリックする。
2.  **[正常系]** お気に入り状態がトグルされ、元のページにリダイレクトされる。

#### 3.4.2. お気に入り一覧機能

##### 機能概要
ユーザーが自分のお気に入りに登録した書籍を一覧表示する機能。

##### 画面遷移
1.  **[開始]** ユーザーがヘッダーの「お気に入り」リンクをクリックする。
2.  お気に入り一覧画面 (`/favorites`) に遷移し、お気に入り登録した書籍が10件ずつのページネーションで表示される。
...
##### 機能概要
ユーザーが他のユーザーのレビューに「いいね」を登録、または解除する機能。

##### 画面遷移
1.  **[開始]** ユーザーが書籍詳細ページでレビューの「いいね」ボタンをクリックする。
2.  **[正常系]** いいね状態がトグルされ、元のページにリダイレクトされる。
...
##### 機能概要
レビューの平均評価が高い順に書籍トップ10をランキング形式で表示する機能。

##### 画面遷移
1.  **[開始]** ユーザーがヘッダーの「ランキング」リンクをクリックする。
2.  ランキング画面 (`/ranking`) に遷移し、レビューの平均評価が高い順に書籍トップ10が表示される。
